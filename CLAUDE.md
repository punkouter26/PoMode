...
One process: `PoMode.API` hosts the Blazor WASM client (`PoMode.Client`), the REST endpoints, a SignalR hub (`/hubs/analysis`), and the background analysis worker. `PoMode.Shared` holds DTOs and the source-generated `PoModeJsonContext`.
...
- Tests share `TestCommon`'s `AppFixture`, which boots the real app; E2EUI uploads real WAVs generated by `TestAudio`.
