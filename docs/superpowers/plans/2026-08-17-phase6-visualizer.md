...
**Behaviour (§7):** three `AudioBufferSourceNode`s started sample-synchronised through `GainNode`s. Full Mix / Solo Vocals / Solo Backing switch by 50 ms `linearRampToValueAtTime` gain ramps — no pops, and **position is preserved** across a mode switch (the sources are never restarted, only the gains move). The scrubber reads the Web Audio clock on `requestAnimationFrame` and pushes it into `canvas.setPlayhead`; the canvas never owns the clock. `mixer.js` imports `setPlayhead` from `canvas.js` directly — both resolve to the same module URL, so they share the same instance — which keeps playback off the Blazor render path entirely. Clicking the canvas calls `seek`.

**Three traps hit while building this — all fixed, all worth remembering:**
1. **A Razor string attribute is a literal, not an expression.** `JobId="_status!.JobId"` passed the nine-character string `_status!.JobId`, producing requests to `/api/analysis/_status!.JobId/stems/mix`. It needs `JobId="@_status!.JobId"`. Nothing failed loudly: the mixer honestly reported `unavailable`.
2. **`@ref` is assigned *during* render, so a parent cannot gate on it.** `@if (_canvas is not null)` never became true because nothing re-rendered after the assignment. `AnalysisCanvas` now raises `OnReady` with its `ElementReference` after first render; the `EventCallback` dispatch re-renders the parent for free. The parent must also clear the stored element when the canvas is removed, or the mixer initialises against a dead element.
3. **The transport clock can read negative.** `startSources` schedules `when = currentTime + 0.02` for sample-sync, so between `play()` and that moment `currentTime - startedAt` is negative and the playhead jumps backwards. Floor the elapsed time at zero: before `when`, nothing has been heard, so the position is still the offset.

**Status vocabulary:** `idle` → `loading` → `ready` | `unavailable`, then `playing` / `paused` / `ended`. Only the transport verbs set it; `seek` and `setMode` refresh the mirrored time/mode but must **not** overwrite the status, or `ready` is destroyed by the first seek.
