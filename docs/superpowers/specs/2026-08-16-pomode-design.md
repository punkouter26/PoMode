...
**Mock-data banner status — correcting the plan's optimism (see §13.9 for the Phase 8 update).** The Phase 5 plan predicted the "USING MOCK DATA" banner would switch off. It does **not** yet, and the reason is not chords. `FakeChordRecognizer` is now unreachable in production (a real, unconditionally-available recogniser is registered ahead of it), so no fake *chord* executor is ever selected. But `MockDataState.PlanContainsFakeExecutor` looks at all four stages, and `FakeStemSeparator`/`FakePitchTracker` are still selected whenever their ONNX models are absent — which is the case in the E2EUI fixture by design (`Models:AutoDownload=false`, isolated `Models:RootPath`, so browser tests stay fast and network-free) and on this box, where only `htdemucs_fp16weights.onnx` has been fetched. The banner going dark is gated on model availability, a Phase-4 concern, not on Phase 5. Related cleanup left undone: `FakeChordRecognizer` is now dead in production yet still registered in `Program.cs` and used as a stand-in in unit/integration tests.

### 13.9 Phase 8 rulings (browser tier — ClientDelegated)

**Measured browser capability on this machine (headless Chromium via this repo's Playwright), probed before planning:** `navigator.gpu` present but `requestAdapter()` returns **null** even with `--enable-unsafe-webgpu --enable-features=Vulkan`; WebAssembly and WASM SIMD present; WASM threads absent (the page is not cross-origin isolated). Consequence: **WASM SIMD is the verified Tier 2 path; WebGPU is opportunistic and unverified here.** The client probes for a real adapter at page load and uses WebGPU when one exists, else WASM — the tests prove the WASM path because it is the one that always works. No COOP/COEP headers were added: threads would change response headers app-wide for a 230 KB model that does not need them.

**Real in-browser inference was achieved, not simulated.** `ClientDelegatedFlowTests` uploads a 440 Hz tone to an app booted in simulated Azure mode (`WEBSITE_INSTANCE_ID` set — the tier's real target scenario, where §4 hard-disables local ONNX), and headless Chromium runs the real pinned `nmp.onnx` through onnxruntime-web 1.27.0 on WASM SIMD, decodes with the ported decoder, and POSTs the notes. The completed job's plan still records `ClientDelegatedPitchTracker` and the notes contain A4 (MIDI 69). The full flow runs in ~10 s.

**Per-job availability changed `ExecutionPlanner`.** Tier 2 exists only for a job whose browser declared it can infer (`POST /api/analysis?clientCanInfer=true`, set after the client's own probe). `PlanAsync(browserCanInfer, ct)` filters the ClientDelegated candidates; the no-argument overload means "no browser help" and behaves exactly as before.

**Placeholder ranking — a real defect found while wiring the tier.** `FakePitchTracker` is Local-tier and always available, so under pure tier ranking a browser doing real inference could **never** be selected in the real app: Tier 2 was unreachable by construction. `IStageExecutor.IsPlaceholder` (default false, true on the three `Fake*` stand-ins) now ranks placeholders after every real free tier but **before Cloud**: mock data must lose to real inference, and must still beat silently spending the user's money. Planning and mid-run fallback share one `EffectiveRank` so they cannot disagree. Corollary: the E2EUI base fixture sets `Tier2:Enabled=false`, because a capable headless browser now legitimately outranks the fake tracker and the older browser tests assert the fake's deterministic output.

**The decoder parity contract is enforced by one committed fixture.** `tests/TestCommon/basic-pitch-decoder.fixture.json` was generated once from `BasicPitchDecoder` and covers the edge cases (re-onset split, sub-58 ms drop, run-to-end). The C# unit test asserts the decoder still reproduces it exactly; `DecoderPortTests` runs the served `basic-pitch-decoder.js` on the same file in real Chromium and asserts note-for-note agreement. A divergence on either side fails a test that names the note.

**Client-result validation limits (§4 step 3 made concrete, shipped in Task 2):** ≤ 20,000 notes; MIDI pitch 21–108; velocity 1–127; `StartSec` ≥ 0 and within track duration + 1 s slack; `DurationSec` in (0, 30]; every number finite. The **whole payload** is rejected on the first bad note rather than filtered — a partially-accepted analysis silently omits notes with nothing telling the user, which is worse than a clean 400 naming the offending index.

**onnxruntime-web is served from our own origin, and the "bundle" build lied about being one.** `/web-runtime/{asset}` serves a fixed allow-list (`ModelCatalog.WebRuntime`) — onnxruntime-web 1.27.0 pinned to immutable npm-versioned jsdelivr URLs, SHA-256-verified by `ModelRegistry`, downloaded on first use, never committed, no CDN in the serving path. Found empirically when the first flow-test run failed: `ort.all.bundle.min.mjs` does **not** embed its Emscripten glue — it fetches `ort-wasm-simd-threaded.jsep.mjs` from `wasmPaths` even for the plain `wasm` execution provider — so both glue modules are catalog assets. The web-runtime list is deliberately not merged into `ModelCatalog.All`: that list drives startup warmup and /diag, and ~40 MB of browser runtime should download on first Tier 2 use, not on every server start.

**Azure deployment caveat, stated rather than implied.** In Azure mode the endpoint serves only files already on disk (`ModelRegistry` refuses downloads there, correctly — but serving a file that exists is exactly Tier 2's point in Azure). A real Azure deployment therefore needs the web-runtime assets shipped alongside the app by the deploy pipeline; until then the tier is honestly absent there. The flow test pre-seeds its models directory the same way and so proves the serving path Azure would use, with zero network in the loop.

**Mock-data banner (§13.8 update):** on this box the banner now goes dark for real uploads — `nmp.onnx` has been fetched, so pitch tracking runs the real local ONNX tier, chords are real (§13.6), and only stem separation still falls to `FakeStemSeparator` when `htdemucs` is absent from the active models root. In the standard E2EUI fixture the banner stays on by design (isolated empty models root).

**Remaining after Phase 8 — all three tiers now exist.** Left open, with reasons recorded: the §13.4 artifact read/write race (partially mitigated: visual/notes/chords/result and web-runtime reads are byte copies, but pipeline writes still race `PhysicalFile`-era assumptions under parallel load); the `Jobs:RootPath` deploy blocker; cloud pitch tracking (no Replicate model with a verifiable `NoteEvent[]` output contract was identified — §13.8); chord-quality improvements (§13.6); and `FakeChordRecognizer` cleanup (§13.8).

### 13.10 Chord-quality fixes (post-Phase 8 — the two fixes §13.6 named)

Both fixes §13.6 prescribed were implemented and re-measured against the same track with the same harness; the baseline reproduced §13.6's numbers exactly (75 spans, 1.7 % N, mean score 0.669, A# 10.2 s, A#m 8.5 s) before anything changed.

**(a) Resolution: resample + log-frequency mapping.** `ChromaExtractor.Compute` now resamples to 22.05 kHz and uses an 8192-sample window (~2.7 Hz bins, 4× finer than the old 4096-at-44.1 kHz), and `Frame` maps each FFT bin to its *fractional* semitone position, splitting magnitude linearly between the two neighbouring semitones instead of hard-rounding a smeared bin into one pitch class. The gathered band is capped at 2 kHz — an 8 kHz cap was measured and was worse on every metric (mean score 0.693 vs 0.730, N 4.6 % vs 2.2 %), confirming high harmonics pollute rather than inform 12-bin chroma.

**(b) Beat-synchronous segmentation.** `TempoEstimator.EstimateGrid` extends the Phase 4 estimator with beat *phase* (comb alignment of the detrended onset envelope; the phase leads the audible onset by up to ~0.09 s — two envelope windows — which is under one chroma frame and therefore harmless). `ChordSegmenter` gained a beat-synchronous overload: each beat interval takes the majority label of its frames and boundaries fall exactly on the grid, so the 0.5 s duration floor no longer exists on this path. When grid confidence is low (sustained pads, silence — material with no beats), it falls back to the duration-floor overload unchanged, which is also what keeps the synthetic-chord integration fixtures deterministic.

**Re-measured result (2017_LonelyHill2.mp3, same harness):** grid 119.7 BPM at confidence 1.0; the semitone-neighbour ghosts §13.6 diagnosed are gone — **A# 10.2 s → 0.0 s, A#m 8.5 s → 1.5 s**, Fm/Gm/C# vanished; mean match score 0.669 → 0.730; min span is now exactly one beat (0.50 s) and the median two (1.00 s), i.e. boundaries are musically aligned instead of floor-absorbed.

**What changed that is *not* provably better:** with the smear removed, several long windows flipped quality — Am 18.8 s → ~1 s and Dm 17.0 s → ~0.5 s became A and D, and F# grew to ~24 s where F#m (the diatonic vi of the A-major reading) would be expected. The histogram is now almost purely A-major diatonic (A, E, D, F#, C#m, Bm), which is *more* internally consistent than before, but without ground truth the major/minor calls remain the known 12-bin-chroma weak spot (§13.6's seventh-exclusion argument applies equally to thirds in dense mixes). The honest verdict moves from "usable, not clean" to "clean boundaries, trustworthy roots, qualities still to be taken with salt".
